...
function pullImage(): array
{
    $image = trim($_POST['image'] ?? '');
    if ($image === '') {
        throw new RuntimeException('Image reference is required.');
    }

    if (!preg_match('/^[a-z0-9][a-z0-9._\/@:-]*$/i', $image)) {
        throw new RuntimeException('Image reference contains invalid characters.');
    }

    $result = runCommand(['docker', 'pull', $image]);

    if ($result['exitCode'] !== 0) {
        throw new RuntimeException($result['error'] ?: 'Failed to pull image.');
    }
