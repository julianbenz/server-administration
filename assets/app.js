...
function setupForms() {
    ...
    if (pullImageForm) {
        pullImageForm.addEventListener('submit', (event) => {
            event.preventDefault();
            handlePullImage(pullImageForm);
        });
    }
...
function handlePullImage(form) {
    const feedback = document.getElementById('pull-image-feedback');
    if (!feedback) {
        return;
    }
    feedback.textContent = 'Pulling image...';
    feedback.classList.remove('error', 'success');

    const formData = new FormData(form);

    fetch(`${apiBase}?action=pull_image`, {
        method: 'POST',
        body: formData,
    })
        .then((response) => response.json())
        .then((data) => {
            if (!data.success) {
                throw new Error(data.message || 'Unable to pull image.');
            }
            feedback.textContent = data.message || 'Image pulled successfully.';
